...
enum States{start_point, init , press_PA2, release_PA2,  press_PA1, press_PA1hold, PINA7_press, Locking} state;
unsigned char TOTAL; 
    ...
    switch(state){
        ...
        case init:
            if(button == 0x04){
                ...
                state = PINA7_press;
            }else if(PORTB == 0x01 && button ==0x04){
                state = press_PA2;
            }
            else
            {
        ...
        case press_PA1:
            if((button == 0x02 || button == 0x00) && PORTB == 0x01){
                state = Locking;

            }
            else if((button == 0x02 || button ==0x00) && PORTB ==0x00)
            {
                state = press_PA1hold;
